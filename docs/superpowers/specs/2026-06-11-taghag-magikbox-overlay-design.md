# Taghag Magikbox Root Overlay Design
...
repository to install the provider, Essentia, Magikbox, and advanced-cue
...
untracked work, including partial Postman collections, Magikbox modules,
...
- Magikbox pgvector-backed analysis, similarity, crate, correction, and mapping
  modules.
...
- Analysis timestamp.
- Raw source payload.
- Normalized features used by Magikbox.
...
## Durable Magikbox Model

Supabase/Postgres is the durable authority for Magikbox state. Local SQLite may
...
- Avoid duplicate competing tables for the same Magikbox concept.
...
- `taghag-intel magikbox recompute`: derive durable vectors.
- `taghag-intel magikbox similar`: query nearest tracks.
- `taghag-intel magikbox crate`: render a neighborhood playlist.
- `taghag-intel magikbox correct`: persist explicit human feedback.
...
- Magikbox policy, vector normalization, correction preservation, pgvector
...
- Magikbox guide explaining vectors, provenance, corrections, and crates.
...
Postman credentials and Essentia analyzer to durable Magikbox and advanced-cue
components.
