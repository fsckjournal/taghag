...
create table public.mp3_file (
  ...
  unique (id, owner_user_id),
  constraint mp3_file_codec_check check (codec = 'mp3'),
  constraint mp3_file_identity_confidence_check
    check (identity_confidence is null or identity_confidence between 0 and 1),
  constraint mp3_file_size_bytes_check check (size_bytes is null or size_bytes >= 0),
  constraint mp3_file_duration_s_check check (duration_s is null or duration_s >= 0),
  constraint mp3_file_bitrate_kbps_check check (bitrate_kbps is null or bitrate_kbps > 0)
);

create table public.mp3_observation (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null references auth.users(id) on delete cascade,
...
create table public.dj_tag (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint dj_tag_mp3_file_owner_fk
    foreign key (mp3_file_id, owner_user_id)
    references public.mp3_file(id, owner_user_id)
    on delete cascade,
...
create table public.tag_evidence (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint tag_evidence_mp3_file_owner_fk
    foreign key (mp3_file_id, owner_user_id)
    references public.mp3_file(id, owner_user_id)
    on delete cascade,
...
create table public.quality_check (
  ...
  constraint quality_check_mp3_file_owner_fk
    foreign key (mp3_file_id, owner_user_id)
    references public.mp3_file(id, owner_user_id)
    on delete cascade
);
...
create table public.crate_track (
  ...
  constraint crate_track_crate_owner_fk
    ...
    on delete cascade,
  constraint crate_track_mp3_file_owner_fk
    foreign key (mp3_file_id, owner_user_id)
    references public.mp3_file(id, owner_user_id)
    on delete cascade,
...
create index mp3_file_owner_file_key_idx on public.mp3_file(owner_user_id, file_key);
create index mp3_file_owner_checksum_sha256_idx on public.mp3_file(owner_user_id, checksum_sha256);
create index mp3_file_owner_checksum_prefix_idx on public.mp3_file(owner_user_id, checksum_prefix);
create index mp3_file_owner_filename_idx on public.mp3_file(owner_user_id, filename);
create index mp3_observation_owner_import_run_idx on public.mp3_observation(owner_user_id, import_run_id);
create index mp3_observation_owner_mp3_file_idx on public.mp3_observation(owner_user_id, mp3_file_id);
...
create index tag_evidence_owner_file_fetched_idx on public.tag_evidence(owner_user_id, mp3_file_id, fetched_at desc);
create index quality_check_owner_file_checked_idx on public.quality_check(owner_user_id, mp3_file_id, checked_at desc);
...
create trigger set_mp3_file_updated_at
before update on public.mp3_file
for each row execute function public.set_updated_at();

create trigger set_mp3_observation_updated_at
before update on public.mp3_observation
for each row execute function public.set_updated_at();
...
grant select on public.mp3_file to authenticated;
grant select on public.mp3_observation to authenticated;
...
grant select, insert, update, delete on public.mp3_file to service_role;
grant select, insert, update, delete on public.mp3_observation to service_role;
...
alter table public.mp3_file enable row level security;
alter table public.mp3_observation enable row level security;
...
create policy "authenticated_select_mp3_file"
  on public.mp3_file
  for select
  to authenticated
  using (owner_user_id = (select auth.uid()));

create policy "authenticated_select_mp3_observation"
  on public.mp3_observation
  for select
  to authenticated
  using (owner_user_id = (select auth.uid()));
