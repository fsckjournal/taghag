...
10. Load any Essentia or Cuecifer sidecars into `track_analysis` only after the parent `audio_file` rows exist, using the existing unique constraint on `(owner_user_id, audio_file_id, schema_name, source_artifact_sha256)` for idempotency.
